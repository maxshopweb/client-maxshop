...
class VentasService {
  ...
  /** Exporta ventas filtradas a Excel ERP (mismo formato FTP). */
  async exportVentasExcel(filters: Omit<IVentaFilters, 'page' | 'limit' | 'order_by' | 'order'> = {}): Promise<Blob> {
    const response = await axiosInstance.post('/ventas/export-excel', filters, {
      responseType: 'blob',
      validateStatus: () => true,
    });
